...
public class Solution3 {
    public static void main(String[] args) {
        Arrays.stream(solution(new String[]{"AAA", "BBB", "CCC", "DDD"})).forEach(System.out::println);
        Arrays.stream(solution(new String[]{"aBc","AbC"})).forEach(System.out::println);
    }

    public static String[] solution(String[] strArr) {
        return IntStream.range(0, strArr.length)
                .mapToObj(i -> {
                    if (i % 2 == 0) return strArr[i].toLowerCase();
                    else return strArr[i].toUpperCase();
                })
                .toArray(String[]::new);
    }
}
